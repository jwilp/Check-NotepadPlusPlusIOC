...
$runningProcs = Get-Process -ErrorAction SilentlyContinue | Where-Object {
    $suspiciousProcesses -contains ($_.ProcessName.ToLowerInvariant())
}
...
if (Test-Path -LiteralPath $nppPluginPath) {
    $pluginDirs = Get-ChildItem -LiteralPath $nppPluginPath -Directory -Force | Select-Object -ExpandProperty Name
